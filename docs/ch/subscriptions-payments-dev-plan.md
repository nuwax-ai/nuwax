...
| 文件 | 变更类型 | 变更内容 |
| --- | --- | --- |
| `src/services/menuService.ts` | 追加 | 新增 10 个菜单 code → icon 映射（含 MorePage 三个菜单） |
| `src/routes/index.ts` | 追加 | 新增 9 条路由（组件资源子页面 + 资源定价 + 订阅 + MorePage） |
...
| 子页面路由可访问 | 浏览器直接访问 `/space/xxx/plugin-workflow` 等路径正常渲染 | ✅ 9 条路由已配置 |
