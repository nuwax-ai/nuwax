...
# 查看所有远程分支（包括 GitHub、GitLab 和 Gitee）
git branch -a
```
...
```bash
# 切换到指定分支并跟踪 Gitee 的对应分支
git checkout -b <local-branch> gitee/<remote-branch>

# 或者如果本地分支已存在
git checkout <local-branch>
git branch --set-upstream-to=gitee/<remote-branch> <local-branch>
```

## 多远程源工作流程

由于项目同时配置了 GitHub (origin)、GitLab (gitlab) 和 Gitee (gitee) 三个远程源，你可以根据需要选择推送到哪个远程源。
...
# 方法 2: 使用脚本一次性推送
git push origin <branch-name> && git push gitlab <branch-name> && git push gitee <branch-name>

# 方法 3: 使用 git remote 命令获取所有远程源
for remote in $(git remote); do git push $remote <branch-name>; done
```

### 设置默认推送目标

如果你希望默认推送到某个远程源，可以修改分支的上游分支：
...
# 设置当前分支的上游分支为 Gitee
git branch --set-upstream-to=gitee/<branch-name> <branch-name>

# 之后可以直接使用 git push，会自动推送到设置的远程源
git push
```
...
如果使用 HTTPS 方式，GitLab 和 Gitee 都会要求输入用户名和密码（或 Personal Access Token）。
...
# Gitee
git remote set-url gitee git@gitee.com:nuwax/nuwax.git
```
...
# 将 gitee 重命名为 gitee-backup
git remote rename gitee gitee-backup
```
...
- 如果使用 SSH，确认 SSH 密钥已添加到对应平台
- Gitee 需要在仓库设置中添加协作者权限
...
# 合并多个远程源的更新
git merge origin/main gitlab/main gitee/main
```
...
[remote "gitee"]
    url = https://gitee.com/nuwax/nuwax.git
...
3. **定期同步**：保持多个远程源的代码同步，避免冲突
4. **多平台备份**：利用 GitHub、GitLab、Gitee 实现代码多地备份，提高代码安全性
5. **分支管理**：为不同远程源使用不同的分支命名规范（可选）
6. **Gitee 镜像**：对于国内访问，Gitee 通常速度更快，可以作为主要镜像源
...
- [Gitee 私人令牌](https://gitee.com/profile/tokens)
