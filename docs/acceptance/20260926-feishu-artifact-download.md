...
## 卡片 F-DOWNLOAD（已按禅道产品 3 全状态 2497 条标题/步骤及近期历史核对，未发现明确等价单）
...
## 独立审阅与集成
- root 对商业 IPC 逐跳凭据、源与最终 URL 后缀/MIME、旧宿主回退、取消及 await 链路独立检查，无阻塞问题；独立执行 3 文件 / 46 项原生保存用例通过。
- 前端集成 1240e7e10 + c548362f7；客户端集成 c2ba72bb + 6ba6757f。客户端集成后同 46 项再次通过，overlay 同步与 pin 检查通过。
