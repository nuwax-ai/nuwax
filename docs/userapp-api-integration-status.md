...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | 用户项目分页查询（常规/全栈/网页应用） | `POST /api/user-project/page-query`<br>`POST /api/user-project/tab/page-query`（2026-09-08 新接口，附带各项目会话列表） | 已完成 | ✅ | service `src/pages/AppDevPro/services/appDevPro.ts`（page-query `:21` / tab `:36`）；消费 `src/pages/SpaceProjectManage/index.tsx:66,80`（tab+关键词，三类并行合并，走 page-query）、`src/layouts/DynamicMenusLayout/NewHomeSection/components/ProjectPanel/index.tsx`（**已切 tab/page-query**，children=conversations 渲染+点击跳转+相对时间，2026-09-08） | mock：`mock/userProjectAPI.ts:68`（dev 供数，仅 page-query）；tab 接口实测契约：记录主键为 `projectId`（非 id）、无 publishStatus、`conversations: ConversationInfo[]`，类型 `UserProjectTabItem`（`src/pages/AppDevPro/type.ts`） |
...
| 5 | 全栈应用：获取当前用户最新会话 | `GET /api/userapp/conversation/{id}` | 已完成 | ✅ | service `appDevPro.ts` `apiUserAppLatestConversation`；消费 `SpaceProjectManage/index.tsx` `handleOpenProject`——打开项目先取最新会话、携带 conversationId 跳 `app-pro`（1472047c7），取不到不阻塞进入 | 返回结构契约未细化，调用侧防御式取 conversationId/id |
| 6 | 常规项目：获取当前用户最新会话 | `GET /api/user-project/conversation/{id}` | 已完成 | ✅ | service `apiUserProjectLatestConversation`；消费同 #5（handleOpenProject 按项目类型分流） | ProjectPanel「项目下会话列表（多条）」已由 #1 tab/page-query 的 conversations 字段供给（2026-09-08 接入，children 不再恒空） |
...
| 30 | 导出项目（全栈、常规） | `GET /api/computer/static/download-all-files` | 已完成 | ✅ | `src/services/vncDesktop.ts:158 apiDownloadAllFiles`（浏览器直下）；消费 `src/pages/Chat/hooks/useChatFiles.ts:313`、`useLocalDirectoryFiles.tsx:502`、`AppDevPro/index.tsx:931`、`ConversationAgent/index.tsx:1229`、`EditAgent/index.tsx:884` | 老接口，wiki 仅确认适用范围扩到全栈/常规项目 |
