...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | 用户项目分页查询（常规/全栈/网页应用） | `POST /api/user-project/page-query` | 已完成 | ✅ | service `src/pages/AppDevPro/services/appDevPro.ts:17`；消费 `src/pages/SpaceProjectManage/index.tsx:66,80`（tab+关键词，三类并行合并）、`src/layouts/DynamicMenusLayout/NewHomeSection/components/ProjectPanel/index.tsx:85` | mock：`mock/userProjectAPI.ts:68`（dev 供数） |
...
| 6 | 常规项目：获取当前用户最新会话 | `GET /api/user-project/conversation/{id}` | 已完成 | ✅ | service `apiUserProjectLatestConversation`；消费同 #5（handleOpenProject 按项目类型分流） | ProjectPanel children「会话列表（多条）」接口不在本契约，children 仍恒空等后端 |
