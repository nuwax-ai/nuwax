...
var require_websandbox = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function webpackUniversalModuleDefinition(root, factory) {
		...
		return (() => {
			...
			(() => {
				__webpack_require__.n = (module$3) => {
					var getter = module$3 && module$3.__esModule ? () => module$3["default"] : () => module$3;
			...
			(() => {
				...
				* */
				...
				const defaultOptions = { allowedSenderOrigin: void 0 };
				class Connection {
					constructor(postMessage, registerOnMessageListener, options = {}) {
						...
						this.incrementalID = Math.floor(Math.random() * 1e5);
					...
					*/
					onMessageListener(e) {
						...
						if (allowedSenderOrigin && e.origin !== allowedSenderOrigin && !isIE11) return;
						if (data.type === TYPE_RESPONSE) this.popCallback(data.callId, data.success, data.result);
						else if (data.type === TYPE_MESSAGE) this.callLocalApi(data.methodName, data.arguments).then((res) => this.responseOtherSide(data.callId, res)).catch((err) => this.responseOtherSide(data.callId, err, false));
						else if (data.type === TYPE_SET_INTERFACE) {
							this.setInterface(data.apiMethods);
							this.responseOtherSide(data.callId);
						} else if (data.type === TYPE_SERVICE_MESSAGE) this.callLocalServiceMethod(data.methodName, data.arguments).then((res) => this.responseOtherSide(data.callId, res)).catch((err) => this.responseOtherSide(data.callId, err, false));
					}
					...
					*/
					setInterface(remoteMethods) {
						...
						(_a = this._resolveRemoteMethodsPromise) === null || _a === void 0 || _a.call(this);
					}
					getMethodsFromInterface(api) {
						return Object.keys(api).reduce((acc, key) => {
							if (typeof api[key] === "object") acc.push(...this.getMethodsFromInterface(api[key]).map((subKey) => `${key}.${subKey}`));
							else acc.push(key);
							return acc;
						}, []);
					}
					...
					*/
					callLocalApi(methodName, args) {
						...
						if (!method) throw new Error(`Local method "${methodName}" is not registered`);
					...
					*/
					callLocalServiceMethod(methodName, args) {
						...
						if (!method) throw new Error(`Service method ${methodName} is not registered`);
						return Promise.resolve(method.call(this, ...args));
					}
					/**
					...
					*/
					responseOtherSide(id, result, success = true) {
						if (result instanceof Error) result = [...Object.keys(result), "message"].reduce((acc, it) => {
							acc[it] = result[it];
							return acc;
						}, {});
					...
					*/
					popCallback(id, success, result) {
						...
						if (!callback) return;
						if (success) callback.successCallback(result);
						else callback.failureCallback(result);
					...
					*/
					constructor(localApi, options) {
						...
						this.promise = new Promise((resolve) => {
							...
							this.connection.setServiceMethods({ iframeInitialized: () => {
					...
					validateOptions(options) {
						var _a;
						if (options.frameSrc && (options.frameContent || options.initialStyles || options.baseUrl || options.codeToRunBeforeInit)) throw new Error("You can not set both \"frameSrc\" and any of frameContent,initialStyles,baseUrl,codeToRunBeforeInit options");
						if ("frameContent" in options && !((_a = options.frameContent) === null || _a === void 0 ? void 0 : _a.includes("<head>"))) throw new Error("Websandbox: iFrame content must have \"<head>\" tag.");
					}
					_prepareFrameContent(options) {
						var _a, _b, _c;
						let frameContent = (_a = options.frameContent) !== null && _a !== void 0 ? _a : "";
						if (options.codeToRunBeforeInit) frameContent = (_b = frameContent.replace("<head>", `<head><script>${options.codeToRunBeforeInit}<\/script>`)) !== null && _b !== void 0 ? _b : "";
						frameContent = (_c = frameContent.replace("<head>", `<head><script>${frame_bundle_default()}<\/script>`)) !== null && _c !== void 0 ? _c : "";
						if (options.initialStyles) frameContent = frameContent.replace("</head>", `<style>${options.initialStyles}</style></head>`);
						if (options.baseUrl) frameContent = frameContent.replace("<head>", `<head><base target="_parent" href="${options.baseUrl}"/>`);
						return frameContent;
					}
					createIframe() {
						var _a;
						...
						if (!container) throw new Error("Websandbox: Cannot find container for sandbox " + container);
						...
						if (this.options.allowFullScreen) frame.allowFullscreen = true;
					...
					run(codeOrFunction) {
						if (codeOrFunction.name) return this._runFunction(codeOrFunction);
...
//#endregion
export default require_websandbox();

//# sourceMappingURL=websandbox-DR5_1T86.js.map