...
export const MOCK_SCENARIOS: MockScenario[] = [
  ...
  },
  {
    ...
    label: '折叠效果全景演示',
    description:
      '复刻真实任务形态：多轮思考 × 工具组 × 终端执行 × 长正文——轮次工作轨迹整体进入折叠区，header 展示「工具调用数 · 消息数 · 已工作时长」，最终 summary 常显在折叠区下方（建议 speed 2~5 观看流式过程）',
    verifies:
      '轮次折叠：header 三项指标 + 终态默认收起仅 summary 常显 + 展开可按原顺序回看思考/工具/终端轨迹',
    events: [
      think(
      ...
      chat(
        '命令执行完成：输出 demo-42、退出码 0。本次任务经历了 4 轮思考与 4 次工具调用（含 1 次终端执行），全部过程已按发生位置折叠进轮次折叠区，展开可回看完整轨迹。',
        true,
      ),
      finalResult(
        true,
  ...
  },
  {
    ...
    label: '终态执行过程聚合',
    description:
      '多轮「正文-工具组」交错的长任务：流式中过程实时展示；终态后全部工作轨迹收进轮次折叠区（默认收起），只展示最终 summary（workbuddy 式终态）',
    verifies:
      '轮次折叠：header 展示 4 次工具调用 + 消息数 + 已工作时长；终态默认收起、仅 summary 常显；展开后中间正文按原顺序完整可见（历史回放一致）',
    events: [
