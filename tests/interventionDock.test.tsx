 ...
 * - unshift 顺序：最新（sortKey 最大）在 front，先处理
 * - front 提交移除后，下一个顶上
 */
...
describe('intervention DockPanel 交互', () => {
  ...
  it('unshift 顺序：最新（sortKey 最大）在 front，先被处理', () => {
    const onRespondAcp = vi.fn();
    // sortKey: a1=1（最早）, a2=3（最新）→ reversed 后 a2 在 front
    render(
      <DockPanel
    ...
    // DOM 顺序 = reversed 渲染顺序，cards[0] 是 front（最新 a2）
    fireEvent.click(cards[0]);
    expect(onRespondAcp).toHaveBeenCalledWith(
      expect.objectContaining({ intervention: { id: 'a2' } }),
      expect.anything(),
    );
  });
  ...
  it('front 提交移除后，下一个自动顶上成为新 front', () => {
    ...
    // 模拟 front（a2）被处理 → useActiveInterventionQueue 重新计算后只剩 a1
    rerender(
      <DockPanel
        items={[acp('a1', 1)]}
        onRespondAcpPermission={onRespondAcp}
    ...
    expect(cards[0].textContent).toBe('acp-a1');
  });

  it('审批与 ask 混合时，front 提交按类型分发到正确回调', () => {
    const onRespondAcp = vi.fn();
    const onRespondMcpAsk = vi.fn();
    // sortKey: acp=1, ask=5 → reversed front = ask
    render(
      <DockPanel
    ...
    // front 是 ask（sortKey 5 最大），点击应触发 onRespondMcpAsk
    fireEvent.click(screen.getByTestId('mcp-card'));
    expect(onRespondMcpAsk).toHaveBeenCalled();
    expect(onRespondAcp).not.toHaveBeenCalled();
  });
