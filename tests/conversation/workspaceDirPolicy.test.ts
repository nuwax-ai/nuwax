 ...
 * 全栈应用（UserApp）当前版本仅云端（不支持个人电脑/自定义目录）；
 * 常规项目支持个人电脑+自定义目录；未登记类型走默认能力。
 */
import {
  CLOUD_SANDBOX_ID,
...
describe('workspaceDirPolicy 目录能力策略', () => {
  ...
  it('全栈应用（UserApp）仅云端：不支持个人电脑与自定义目录', () => {
    const policy = getWorkspaceDirPolicy(AgentComponentTypeEnum.UserApp);
