...
## 追修（同日，1b5aa3445 后用户复测「设置改简体中文，刷新/重启变英文」）

- **根因补全**：首修的残留防护只覆盖「无显式标记」分支。用户显式选中文后标记='2'，而 `saveUserLang`（user/update 持久化 user.lang）定义了从未接线，设置面板注释里「i18n/query 带 lang 会同步更新用户偏好」是想当然——账号侧 `user.lang` 永远停在 en-US，`syncLangFromUserInfo` 见显式标记即放行，每次刷新把已选中文顶回英文。
- **语义升级**：`shouldSyncAccountLang` 改为——本地显式选择（'2'）存在时账号侧一律不覆盖；无本地选择时账号语种可补位，en-us 残留仍被忽略。
- **真正持久化**：设置面板保存成功后 `await saveUserLang(lang)`（失败不阻断本地切换）；移除「选中租户默认语言拒绝保存」的错误守卫（后端默认若为 zh-CN 会反过来禁止用户显式选中文）；删除想当然注释。登录页切换未登录不调持久化，本地选择仍生效。
- 追加代价说明：一台设备上的显式选择不会被其它端后改的语种传播更新（本地为准）；「不回写后端」一条由 saveUserLang 接线部分取代（仅用户显式保存时写）。
