...
- 2026-09-11(十九轮,**能力弹窗团队维度个人空间置顶**):用户要求「添加能力」弹窗团队空间维度个人空间排最前(type 判定)。useCapabilityCategories 的 sortSpacesTeamFirst 反转为 sortSpacesPersonalFirst(SpaceTypeEnum.Personal 优先,其余稳定保持接口序);默认选中随 categories[0] 自动变为个人空间(上层兜底 effect 与 defaultSpaceId 均取首位,无需另改)。新增 capabilityModalSpaces.test.tsx(2 用例:个人空间置顶+默认选中首位 key/无个人空间不崩溃)。注:IAB 输入管道当日持续劣化,弹窗 UI 未真机点验,以单测契约为准;门禁 528 项全绿。
- 2026-09-11(二十轮,**能力弹窗专家数据源统一为已发布智能体接口 + 全部页签 + 付费标识**):用户三项要求——① 专家=智能体,系统广场/团队空间都走 /api/published/agent/list;② 团队维度新增「全部」页签,经接口参数 spaceIds 聚合;③ 付费专家展示付费标识。参考口径:系统广场=/square?cate_type=Agent 默认调用(targetType=Agent+targetSubType=ChatBot,**去掉 official**——广场页默认过滤"全部"不发 official);团队空间=/space/:id/space-square?activeKey=Agent(category='Agent'+justReturnSpaceData:true)。实施:SquarePublishedListParams 补 spaceIds?: number[];useCapabilityResources 的 expert.team 由 apiAgentConfigList(空间配置全量)切为 publishedServerAdapter(具体空间=单 spaceId、全部=spaceIds,category=Agent+justReturnSpaceData),expert.system 撤 official;useCapabilityCategories 专家维度团队 pills 首位加「全部」(其余类型不变,个人空间仍在空间中置顶,默认选中=全部);index.tsx 组装 expertSpaceIds(全部=全部空间/具体=单元素)与 spaceId(专家"全部"不回落默认空间);CapabilityCard 专家 paymentRequired&&!subscribed 时 card-meta 展示金色「付费」徽标(.card-paid),新增文案键 CapabilityModal.paid ×5 locale。测试:capabilityModalSpaces 扩至 6 用例(排序/专家全部页签首置/数据源三口径断言);修两处测试坑——spaceIds 内联数组致 hook 重置 effect 无限循环(OOM),SUCCESS_CODE='0000' 非 200。门禁 532 项全绿。
- 2026-09-11(二十一轮,**专家卡完全对齐广场卡样式 + hover 效果 + 收藏**):用户指出「没有完全对齐 /square 卡样式,还有 hover 相关效果」。对齐实施:① 信息一致——CapabilityItem/mapPublishedItem 补 official/convCount/collectCount/collect(SquarePublishedItemInfo 补 official?: boolean),卡片 meta 行徽标体系 .card-badge(官方=金/付费=红/已订阅=绿,color-mix tint),描述两行截断修复(display: box 笔误 →-webkit-box,此前 clamp 失效);专家 tags 行移除(广场卡无此行)。②hover 效果对齐广场卡——卡片悬停/键盘聚焦:主色描边+分层阴影抬升(0.3s);底部操作浮层 .card-action-overlay(对齐 action-box:opacity+z-index 双翻转过渡)悬停浮现「聘请(带消息图标)+收藏星标」覆盖统计行,未浮现不挡点击;统计行常驻(用户/会话/收藏数,#828894 同款)。③ 收藏——onToggleCollect 走 apiCollectAgent/apiUnCollectAgent(targetId),成功 updateItem 就地更新 collect 与 collectCount;星标 aria 收藏/取消收藏。新增文案键 official/collect/uncollect ×5 locale(付费/已订阅复用 Square.SingleAgent 键)。真机验证(IAB 恢复):系统广场专家卡统计 108/188/6+悬浮聘请/取消收藏 ✓;团队空间「全部」页签(默认选中)经 spaceIds 聚合返回个人空间智能体 ✓;pill 顺序 全部/个人空间/测试空间 ✓。门禁 532 项全绿。
- 2026-09-11(二十二轮,**专家卡布局细化:三段式 flex + 交互元素归位**):用户多轮调优定稿——① 聘请按钮移至标题行右端(与标题对齐,hover/键盘聚焦浮现,常驻占位防标题跳动);② 收藏星标与付费标识移至底部统计行右端:收藏 hover 浮现(opacity+pointer-events 门控,占位保留防挤压),付费常驻;③ 付费/已订阅改用 antd Tag color="blue"(.card-paid-tag 归零其自带 margin),官方徽标保持金色 tint 款;④hover 移开复位——卡片 onMouseLeave→onHover(-1) 清除聚焦态(边框/阴影/浮层全部回落,修复聚焦态粘滞)。⑤ 布局根因修复:卡片三段式(header flex:0 0 auto / content flex:1 1 auto + min-height:0 / footer flex:0 0 auto),描述块专家卡常驻(空内容撑住中段),**去掉 .card-expert min-height:150**——该 min-height 干预网格轨道取值致两行描述卡溢出 28px/底部间距错乱,移除后卡片高度恢复内容驱动(实测两行 177/一行 157,零溢出,footer 底部留距统一)。诊断过程:grid-auto-rows/max-content/移除 min-height 对照实验逐项排除,最终按用户指定 min-height 方案收敛。测试 30 项绿 + tsc 零新增;视觉留用户真机确认(IAB 输入管道反复劣化)。
- 2026-09-11(二十三轮,**技能维度对齐专家同款数据链路 + 技能卡简化**):① 数据——技能团队维度由 apiSkillList(空间全量)切为已发布技能接口,与 /space/:id/space-square?activeKey=Skill 同口径(category='Skill'+justReturnSpaceData,具体空间=单 spaceId、全部=spaceIds 聚合);系统广场维度核对与 /square?cate_type=Skill 一致(原样);teamClientAdapter/mapSpaceItem/apiSkillList 清理下线。② 分类——技能团队维度同专家首位加「全部」页签(默认选中,个人空间次之);index.tsx publishedSpaceIds/spaceId 组装扩展至 expert+skill。③ 技能卡简化(用户定稿)——无 footer(无统计/收藏/付费标识)、无收藏(onToggleCollect 保持专家专属智能体收藏接口);选择按钮与置顶按钮并入标题行右端操作簇(.card-head-actions 同簇对齐,悬停浮现);描述回退有内容才渲染。测试:capabilityModalSpaces 扩至 9 用例(资料库维度无全部页签/专家+技能全部页签首置/五组数据源口径断言),修复新用例 spaceIds 内联数组引发的无限循环 OOM(复用稳定常量);门禁 535 项全绿,tsc 零新增。
- 2026-09-11(二十四轮,**技能卡收藏/付费恢复 + 置顶位置定稿**):用户更正「技能也有收藏和付费,需要搞回来」——技能卡恢复 footer(广场技能卡同口径:仅收藏数,隐藏用户/会话数)、收藏星标(hover 浮现,走 apiPublishedSkillCollect/UnCollect 按类型分流)与付费 antd Tag 蓝色(常驻);描述恢复常驻(撑中段 footer 落底)。操作簇定稿:置顶按钮在选择按钮左侧同簇对齐(用户指示「盯住在选择左边」)。门禁 535 项全绿。
