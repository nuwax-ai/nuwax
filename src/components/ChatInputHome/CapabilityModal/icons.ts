 ...
 *   （技能/资料库/上下文文件三种 chip 统一图标形态）
 * FileTextOutlined 同形常量供资料库与上下文文件共用（antd 图标库无官方
 * 字符串出口；升级图标库版本时需人工比对形态）
 */

/** FileTextOutlined 同形（fill 跟随 currentColor，1em 随字号缩放） */
const FILE_TEXT_SVG =
...
export const CAPABILITY_MENU_ICON_SVGS = {
  ...
  /** 资料库：FileTextOutlined 同形 */
  knowledge: FILE_TEXT_SVG,
  /** 上下文文件：与 @ 弹层文件列表行（FileTextOutlined）同形一致 */
  file: FILE_TEXT_SVG,
} as const;
