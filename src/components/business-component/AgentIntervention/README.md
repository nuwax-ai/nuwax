...
```
AgentIntervention/
  index.ts                          # 对外 barrel（见下文）
  types/
    acpIntervention.ts
    mcpAskIntervention.ts
  components/
    ...
    McpAskQuestionCard/
  hooks/
    useAgentInterventionLayer.ts
    useAgentInterventionHandlers.ts
    useActiveInterventionQueue.ts
    useInterventionEscapeKey.ts
  utils/
    processInterventionSsePatch.ts
    parseSseEventEnvelope.ts
    applyAcpPermissionSseEvent.ts
    applyMcpAskToolCallSseEvent.ts
    parseMcpAskToolInput.ts
    parseMcpAskSchema.ts
    mcpAskHydrateMessage.ts
    mcpAskResumeMessage.ts
    interventionTrigger.ts
```
