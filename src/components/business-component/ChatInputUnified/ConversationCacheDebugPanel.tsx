/**
 * 会话调试面板：顶部菜单两档——「缓存遥测」（LRU 实例列表 + 执行中的页面实例分区）
 * 与「环境信息」（页面地址/登录/环境/版本，见 ConversationEnvDebugPanel）。
...
import ConversationEnvDebugPanel from './ConversationEnvDebugPanel';

const cx = classNames.bind(styles);

type DebugTab = 'cache' | 'env';

const TAB_ITEMS: Array<{ key: DebugTab; label: string }> = [
  { key: 'cache', label: '缓存遥测' },
  { key: 'env', label: '环境信息' },
];

const TAB_HEADINGS: Record<DebugTab, { eyebrow: string; title: string }> = {
  cache: { eyebrow: '运行时遥测', title: '会话页面缓存' },
  env: { eyebrow: '环境信息', title: '运行环境' },
};
...
interface ConversationCacheDebugPanelProps {
  /** 弹层是否处于打开态；透传给环境信息分区用于刷新快照 */
  active?: boolean;
}

const ConversationCacheDebugPanel: React.FC<
  ConversationCacheDebugPanelProps
> = ({ active = false }) => {
  const snapshot = useConversationPageCache();
  const [tab, setTab] = useState<DebugTab>('cache');
  ...
  return (
    <section className={styles.panel} data-testid="conversation-cache-debug">
      <div className={styles.masthead}>
        <div>
          <div className={styles.eyebrowRow}>
            ...
            <span className={styles.eyebrow}>{TAB_HEADINGS[tab].eyebrow}</span>
          </div>
          <div className={styles.title}>{TAB_HEADINGS[tab].title}</div>
      ...
      <div className={styles.tabs} role="tablist">
        {TAB_ITEMS.map((item) => (
          <button
            ...
            className={cx(styles.tab, tab === item.key && styles.tabActive)}
      ...
      {tab === 'env' ? (
        <ConversationEnvDebugPanel active={active} />
      ) : (
        <>
          ...
          <div className={styles.execSection}>
            <div className={styles.execHeader}>
              ...
              <span className={styles.execHint}>
                任务状态 CREATE / EXECUTING
              </span>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className={styles.entryMeta}>
                      ...
                      <span>
                        草稿{' '}
                        {entry.draft.hasContent ? entry.draft.textLength : 0}
                        字/
          ...
          <div className={styles.footer}>
            ...
            <span>
              VNC 占用：{snapshot.sharedVncOwnerConversationId ?? '无'}
            </span>
          </div>
        </>
      )}
