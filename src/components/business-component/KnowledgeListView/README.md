...
- 横向资料卡：资料库同款线性文件图标（`RepoFileIcon` 复刻 nuwax-repo-web 资料库的 `docIconFor` 体系——pageType/扩展名 → 类型专属图形与配色，未识别回落默认文档蓝；外层 40px 圆形 tinted 底与技能/专家/连接器网格卡图标口径一致）+ 名称 + 右端相对时间胶囊（仅 recent）+ 悬停「选择」按钮（仅 grid：时间胶囊淡出让位、按钮右端对齐覆盖，不展示文档类型文本；list 不渲染）。
