...
1. **普通文本中间**
   ...
   - 输入: `12dsds212|` (然后输入 `{`)
   - 行为: 不触发（光标前是字母或数字，在普通文本中间）
   - 原因: 只有在合适的上下文中才能插入变量（行首、空格后、变量间等）
...
1. ✅ 检查光标**前**一个字符，如果是字母或数字则阻止（避免在普通文本中触发）
2. ✅ 检查光标**前**的文本，阻止在 `}}` 紧跟后触发（除非后面紧跟 `{{`）
3. ✅ 允许在两个紧邻的变量之间插入（`{{xxx}}{|{{yy}}`）
...
1. 只允许在两个**紧邻**的完整变量之间插入新变量
2. 不允许在普通文本中间（字母、数字后）触发建议
