...
export interface ExpressionEditorSegment {
  type: ExpressionEditorSegmentType;
  index: number;
  objectKey?: string;
  arrayIndex?: number;
}
...
export interface ExpressionEditorParseData {
  ...
  segments: {
    inline: ExpressionEditorSegment[] | undefined;
    reachable: ExpressionEditorSegment[] | undefined;
  };
}
...
export namespace ExpressionEditorParserBuiltin {
  ...
  export const tokenOffset = (line: {
    ...
    if (firstEndTokenOffset === -1) {
      // If no closing brace found after cursor, check if we are inside one
      // But the original logic seems to rely on finding a closing brace?
      // "endChars !== ExpressionEditorToken.FullEnd" check suggests it expects to find it.
      // If we are typing inside {{}}, we might not have closed it yet?
      // Or maybe this parser expects valid full expressions?
    ...
    if (lastStartTokenOffset === -1) return undefined;

    const startChars = content.slice(
      lastStartTokenOffset,
      lastStartTokenOffset + 2,
    );
  ...
  export const extractContent = (params: {
    ...
    const content = lineContent.slice(
      lastStartTokenOffset + 2, // +2 for {{
      firstEndTokenOffset,
    );
    const offset = lineOffset - lastStartTokenOffset - 2; // -2 for {{
    return {
      content,
      offset,
    };
