    ...
    <div className={cx(styles.container, 'flex')}>
      {
        messageInfo.role !== AssistantRoleEnum.USER ?  (
          <>
            <img className={cx(styles.avatar)} src={agentConfigInfo?.icon} alt="" />
            <div className={cx('flex-1')}>
              <div className={cx(styles.author)}>{agentConfigInfo?.name}</div>
              <RunOver />
              <div className={cx(styles['chat-content'], 'radius-6')}>{messageInfo.text}</div>
              <ChatBottomMore
                onCopy={onCopy}
                onDebug={onDebug}
              />
          ...
          <>
            ...
            <div className={cx('flex-1')}>
              ...
              <span className={cx(styles['chat-content'], 'radius-6')}>{messageInfo.text}</span>
            </div>
          </>
        )
      }
