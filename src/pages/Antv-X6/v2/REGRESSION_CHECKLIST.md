...
| 连续添加节点相对偏移 | ✅ |  | 每次偏移 16px，与 V1 一致 |
| 打开面板时重置偏移计数 | ✅ |  | setContinueDragCount(0) |
...
| 关闭配置面板 | ✅ |  |  |
| 节点图标显示 | ✅ |  | returnImg 渲染 SVG |
| 节点名称显示 | ✅ |  | 16px 加粗，颜色跟随节点背景色 |
| 节点名称双击编辑 | ✅ |  | onDoubleClick 触发编辑 |
| 节点名称编辑长度限制 | ✅ |  | maxLength={25} |
| 节点描述显示 | ✅ |  | 单独一行，支持 Popover 展开 |
| 节点描述双击编辑 | ✅ |  | TextArea 编辑模式 |
| 描述保存到节点 description | ✅ |  | onNodeDescChange 回调 |
| 操作菜单（复制/删除/试运行） | ✅ |  | Dropdown 菜单 |
| 试运行按钮（CaretRightFilled） | ✅ |  | Popover 提示"测试该节点" |
| 分割线 | ✅ |  | 头部与内容区之间 |
| 头部两行布局（标题/描述分开） | ✅ |  | row1 标题按钮，row2 描述 |
| 渐变背景 | ✅ |  | 从节点颜色渐变到白色 |
