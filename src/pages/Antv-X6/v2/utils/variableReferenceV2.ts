 ...
 * 生成参数的完整路径键
 */
function generateArgKey(nodeId: number, argName: string, path: string[] = []): string {
...
export function calculateNodePreviousArgs(
  ...
  // 构建反向图
  const reverseGraph = buildReverseGraph(nodeList);
  ...
  predecessorIds.forEach(predId => {
    ...
    if (predNode.type === NodeTypeEnumV2.LoopStart || 
  ...
  if (currentNode.loopNodeId) {
    ...
    if (loopNode && loopNode.innerNodes) {
      ...
      const innerPredecessorIds = findAllPredecessors(nodeId, innerReverseGraph);
...
export function getAvailableVariables(
  ...
  const { previousNodes, argMap } = calculateNodePreviousArgs(nodeId, workflowData);
