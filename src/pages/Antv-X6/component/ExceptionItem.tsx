...
export const ExceptionItem: React.FC<ExceptionItemProps> = ({
  ...
  useEffect(() => {
    setJsonContent(specificContent || DEFAULT_JSON_CONTENT);
  }, [specificContent]);
  ...
  useEffect(() => {
    ...
    const initialValue = {
      ...
      retryCount,
      exceptionHandleType,
      ...(exceptionHandleType === ExceptionHandleTypeEnum.SPECIFIC_CONTENT && {
        specificContent,
      }),
      ...(exceptionHandleType ===
    ...
    // 初始化JSON内容
    if (specificContent) {
      setJsonContent(specificContent);
    }
  }, [
    outerForm,
    name,
  ...
  const handleExceptionTypeChange = (value: ExceptionHandleTypeEnum) => {
    ...
    if (value === ExceptionHandleTypeEnum.SPECIFIC_CONTENT) {
      try {
        ...
        setJsonContent(DEFAULT_JSON_CONTENT);
        if (outerForm) {
          outerForm.setFieldsValue({
            [name]: {
              ...outerForm.getFieldValue(name),
              specificContent: DEFAULT_JSON_CONTENT,
            },
          });
        }
      ...
      <div className={cx(styles.exceptionItemForm)}>
        <div className={cx(styles.exceptionItemRow)}>
          ...
          <Form.Item
            ...
            label="异常处理方式"
            className={cx(styles.exceptionItemFormItem, styles.handleTypeItem)}
          >
            <Select
              size="small"
