...
describe('extractPortSuffix', () => {
  ...
  it('should strip nodeId prefix and -out suffix', () => {
    expect(
      extractPortSuffix(makeNode(10), '10-eval-pass-out'),
    ).toBe('eval-pass');
  });

  it('should work with string nodeId', () => {
    expect(
      extractPortSuffix(makeNode('abc'), 'abc-route-r1-out'),
    ).toBe('route-r1');
  });

  it('should handle portId without nodeId prefix (graceful fallback)', () => {
    expect(extractPortSuffix(makeNode(10), 'eval-pass-out')).toBe(
      'eval-pass',
    );
  });

  it('should handle portId without -out suffix', () => {
    expect(extractPortSuffix(makeNode(10), '10-eval-pass')).toBe(
      'eval-pass',
    );
  });
  ...
  it('should preserve uuid with hyphens in it', () => {
    expect(
      extractPortSuffix(makeNode(10), '10-eval-fail-v1-uuid-out'),
    ).toBe('eval-fail-v1-uuid');
  });
});
...
describe('branchPortY', () => {
  ...
  it('should compute index N (linear progression)', () => {
    ...
    expect(yHeight).toBe(BRANCH_PORT_BASE_Y + (n + 1) * BRANCH_PORT_ITEM_HEIGHT - BRANCH_PORT_STEP);
    expect(offsetY).toBe(BRANCH_PORT_BASE_Y + (n + 1) * BRANCH_PORT_ITEM_HEIGHT);
  });
...
describe('port-layout invariant (handler <-> chip alignment)', () => {
  ...
  it('RouteDecision: default 用 branchPortY(0), route[i] 用 branchPortY(i+1), chip[i] 显式传 portIndex=i+1', () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const chipTopY =
        BRANCH_PORT_BASE_Y + (i + 2) * BRANCH_PORT_ITEM_HEIGHT - BRANCH_PORT_STEP;
