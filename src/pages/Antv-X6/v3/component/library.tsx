...
// 知识库写入节点：选择目标知识库并写入内容，形态参照 TableDataAdd
const KnowledgeInsertNode: React.FC<NodeDisposeProps> = ({
  form, // updateNode,
  type,
  id,
}) => {
  ...
  // 知识库（单选：有且只能选一个，新增时直接替换）
  const onAddedSkill = (item: CreatedNodeItem) => {
    item.type = item.targetType as unknown as NodeTypeEnum; //类型转换
    item.knowledgeBaseId = item.targetId;

    // 单选：直接替换为当前选中的知识库，而非追加
    form.setFieldValue(KBC_FORM_KEY, [item]);
    setIsModified(true);
    // V3: 触发全量保存
    form.submit();
    setOpen(false);
      ...
      <div className="node-item-style">
        <div className="dis-sb margin-bottom">
          ...
          <Form.Item shouldUpdate noStyle>
            {() =>
              // 已选中知识库时隐藏新增按钮（单选）
              !(form.getFieldValue(KBC_FORM_KEY) || []).length ? (
                <Button
                  icon={<PlusOutlined />}
                  size={'small'}
                  onClick={showAdd}
...
export default { KnowledgeNode, KnowledgeInsertNode };
