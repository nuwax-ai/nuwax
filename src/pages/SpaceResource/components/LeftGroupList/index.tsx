...
const LeftGroupList: React.FC<LeftGroupListProps> = ({
  ...
  // 监听组件列表物理真实变更的触发器（比如拉取或删除成功时显式增加的值），以防在 HeaderArea 内的前端过滤触发误刷新
  useEffect(() => {
    ...
    // 之后的每一次组件列表的真实物理变更（如添加、删除等），都必须联动强刷分组列表，保证 toolCount 与分组状态同步
    fetchGroupList(undefined, true);
  }, [refreshTrigger, fetchGroupList]);
  ...
  const titleNode = useMemo(() => {
    return (
      <div className={cx(styles['selection-header-wrap'])}>
        <span>{title || dict('PC.Constants.Space.group')}</span>
        <Button
          type="link"
          icon={<PlusOutlined />}
          onClick={(e) => {
