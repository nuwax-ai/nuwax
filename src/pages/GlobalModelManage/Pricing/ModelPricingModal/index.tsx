...
const ModelPricingModal: React.FC<ModelPricingModalProps> = ({
  ...
  useEffect(() => {
    ...
    const fetchModelOptions = async () => {
      try {
        const result = await apiSystemModelList();
    ...
    try {
      if (editItem) {
        ...
        if (removedIds.length) {
          await Promise.all(
            removedIds.map((id) => apiSystemDeleteModelPricing(id)),
          );
        }
      }
      ...
      for (const tier of tiers) {
        ...
        // 更新档位
        if (tier.id) {
          await apiSystemUpdateModelPricing(payload);
        } else {
          // 新增档位
          await apiSystemCreateModelPricing(payload);
        }
      }
