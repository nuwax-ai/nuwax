 ...
 * title)。iframe 地址来自 GET /api/userapp/domain/list(appId 入参)回包:
 * 优先自定义域名(Custom),无则生产域名(Prod),再回退首条。加载失败或无
 * 可用域名时对齐 /agent/:agentId(ConversationDetails)口径:错误 toast 由
 * 全局请求层弹出,页面退出 loading 后仅展示空态,无专属错误页。左侧会话区
 * 后续按需求迭代。
 */
...
const UserApp: React.FC = () => {
  ...
  // 域名列表:GET /api/userapp/domain/list(appId query)
  ...
  // 失败时全局请求层已 toast,标题缺省不影响 iframe 展示
  const { data: appInfo } = useRequest(() => apiUserAppGetById(appId), {
    refreshDeps: [appId],
  });

  // iframe 域名优先级:自定义域名(Custom)> 生产域名(Prod)> 首条兜底
  ...
  const previewUrl = normalizeDomainUrl(previewEntry?.domain);
