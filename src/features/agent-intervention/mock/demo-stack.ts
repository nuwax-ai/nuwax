/**
 * 开发环境：一次性注入全部干预类型 Mock（固定栏堆叠演示）
 */
import type { MessageInfo } from '@/types/interfaces/conversationInfo';
import { injectAcpPermissionIntoMessageList } from '../acp-permission/mock';
import type { McpAskQuestionMockScenario } from '../mcp-ask/mock';
import { createMockMcpAskInteraction } from '../mcp-ask/mock';
import { isAgentMessage } from '../utils/is-agent-message';

/** Ask 各 UI 形态（不含纯状态态 mock） */
...
/**
 * 按触发顺序注入：ACP 权限 → Ask(单选) → Ask(多选) → Ask(自定义) → Ask(文本) → Ask(向导)
 */
