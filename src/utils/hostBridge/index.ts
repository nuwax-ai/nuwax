...
export const shellAvoid = {
  ...
  TOP: 36,
  /**
   * Win/Linux 内容区（page-container）顶部避让：壳顶行行高 40 但全透明，
   * 图标/菜单字形只占行上部（~34px），内容卡贴字形下沿以减少顶部空白。
   * 比 ROW_H 小、比 0 大——保证卡片不被窗口拖拽带当作可拖拽区吞点击。
   * 2026-09-16 用户反馈：曾对齐顶行提到 40，实测退让过多；09-17 又反馈
   * 28 时字形下沿压着卡片上沿，回加到 32 留出间隙。
   */
  CONTENT_TOP: 32,
